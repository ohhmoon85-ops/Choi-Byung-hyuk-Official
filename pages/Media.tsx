        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {MEDIA_ITEMS.map((item) => (
            <div key={item.id} className="group relative overflow-hidden rounded-lg shadow-sm cursor-pointer">
              {/* Image Container */}
              <div className="aspect-w-16 aspect-h-9 w-full h-64 overflow-hidden bg-gray-200">
                <img 
                  src={item.imageUrl} 
                  alt={item.title} 
                ...
                <div className="absolute inset-0 bg-navy-900/0 group-hover:bg-navy-900/40 transition-colors duration-300 flex items-center justify-center">
                  {item.type === 'video' && <PlayCircle size={48} className="text-white opacity-0 group-hover:opacity-100 transform scale-75 group-hover:scale-100 transition-all duration-300" />}
                  {item.type === 'photo' && <ImageIcon size={48} className="text-white opacity-0 group-hover:opacity-100 transform scale-75 group-hover:scale-100 transition-all duration-300" />}
                  {item.type === 'press' && <FileText size={48} className="text-white opacity-0 group-hover:opacity-100 transform scale-75 group-hover:scale-100 transition-all duration-300" />}
                </div>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6">
                ...
                <h3 className="text-white font-bold leading-tight drop-shadow-md">
                  {item.title}
                </h3>
              </div>
            </div>
